...
export default function OrderSearch({ defaultValue, status }: { defaultValue: string; status?: string }) {
  ...
  function clear() {
    ...
    const params = new URLSearchParams();
    if (status) params.set("status", status);
    router.push(`/dashboard/orders${params.toString() ? `?${params}` : ""}`);
  }
    ...
    <form onSubmit={submit} className="mb-4">
      <div className="relative">
        ...
        <input
          ...
          placeholder="Cari no. order, nama, atau HP..."
