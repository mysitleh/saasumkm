...
export default function PromoForm() {
  ...
  const [form, setForm] = useState({ code:"", type:"PERCENT", value:"", minOrder:"0", maxDiscount:"", usageLimit:"", perCustomerLimit:"", expiresAt:"" });
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      const res = await fetch("/api/dashboard/promos", { method:"POST", headers:{"Content-Type":"application/json"}, body: JSON.stringify({ ...form, code: form.code.toUpperCase(), value: parseInt(form.value), minOrder: parseInt(form.minOrder||"0"), maxDiscount: form.maxDiscount ? parseInt(form.maxDiscount) : null, usageLimit: form.usageLimit ? parseInt(form.usageLimit) : null, perCustomerLimit: form.perCustomerLimit ? parseInt(form.perCustomerLimit) : null, expiresAt: form.expiresAt||null }) });
      const data = await res.json();
      if (!res.ok) setError(data.error||"Terjadi kesalahan.");
      else { setForm({ code:"", type:"PERCENT", value:"", minOrder:"0", maxDiscount:"", usageLimit:"", perCustomerLimit:"", expiresAt:"" }); router.refresh(); }
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-3">
      ...
      <div className="grid grid-cols-2 gap-3">
        <div><label className="block text-xs font-medium text-[var(--ink-muted)] mb-1">Kuota Total <span className="text-[var(--shade-50)]">(opsional)</span></label><input name="usageLimit" type="number" value={form.usageLimit} onChange={handleChange} min={1} className="w-full border border-[var(--border-warm)] rounded-[14px] px-3 py-2 text-sm focus:outline-none focus:ring-[3px] focus:ring-[var(--accent)]/20 focus:border-[var(--accent)]" placeholder="Tanpa batas"/><p className="text-[10px] text-[var(--shade-50)] mt-1">Berapa kali promo bisa dipakai total.</p></div>
        <div><label className="block text-xs font-medium text-[var(--ink-muted)] mb-1">Maks. per Customer <span className="text-[var(--shade-50)]">(opsional)</span></label><input name="perCustomerLimit" type="number" value={form.perCustomerLimit} onChange={handleChange} min={1} className="w-full border border-[var(--border-warm)] rounded-[14px] px-3 py-2 text-sm focus:outline-none focus:ring-[3px] focus:ring-[var(--accent)]/20 focus:border-[var(--accent)]" placeholder="Tanpa batas"/><p className="text-[10px] text-[var(--shade-50)] mt-1">Per nomor HP customer.</p></div>
      </div>
