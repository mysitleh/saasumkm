...
async function main() {
  ...
  const tenant = await prisma.tenant.upsert({
    ...
    create: {
      ...
      slug: "demo",
      description: "Kedai kopi terbaik di kota.",
      address: "Jl. Contoh No. 1, Jakarta",
      phone: "08123456789",
    },
  });

  // Trial subscription PRO supaya semua fitur premium aktif untuk demo.
  const now = new Date();
  ...
  for (const p of products) {
    await prisma.product.upsert({ where: { id: p.id }, update: {}, create: { ...p, tenantId: tenant.id } });
  }

  await prisma.promo.upsert({
    where: { tenantId_code: { tenantId: tenant.id, code: "HEMAT10" } },
    update: {},
    create: { tenantId: tenant.id, code: "HEMAT10", type: "PERCENT", value: 10, minOrder: 50000, maxDiscount: 15000 },
  });

  console.log("✅ Seed complete!");
  console.log("📋 Demo: owner@demo.com / password123 | /store/demo");
  console.log("💎 Subscription: PRO (TRIAL) — semua fitur Phase 2/3 aktif untuk testing");
}
